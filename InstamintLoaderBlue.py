...
class InstamintLoader():
    ...
    def add_usr(self,username,name,email_domain,profile_url=None,password='$2a$10$e7Wvy/dj49K9r4JaLmt/TuTkEtLvE9MGP3Eh7aJ0bLxB4acxxHE2K',disabled=False,bulk=0, \
        ...
        if bulk==0:
            ...
            for n in range(start_bulk, n_rows):
                ...
                self.cur.execute('INSERT INTO USR (ID, EMAIL,USER_NAME,FULL_NAME,IS_DISABLED,PASSWORD,PROFILE_PHOTO_URL,CREATED_BY,CREATED_DT) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)', \
                (n,username+str(n)+ '@' + email_domain,username+str(n),name+str(n),disabled,password,random.choice(InstamintLoader.profile_urls),1,datetime.now()))
                self.commit()
    ...
    def add_erc721token (self, id=1, created_by=1, created_dt=datetime.now(), last_modified_by=None, updated_dt=None, active=True, best_ask=0, best_bid=0, featured=True, fotd=True, public_view=True, \
                        ...
                        meta_data_ipfs_url='dummy data', mint_gas_fee=0, mint_success=True, name='dummy data', title='dummy data', view_public=True, contract_id=1, n_rows=1, token_id=1):
         ...
         for n in range(start_bulk, n_rows + 1):
            ...
            self.cur.execute("INSERT INTO erc721token (ID, CREATED_BY, CREATED_DT, LAST_MODIFIED_BY, UPDATED_DT, DESCRIPTION, "\
                                                      ...
                                                      "CONTRACT_ID, TOKEN_ID) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", \
                                (id, created_by, created_dt, last_modified_by, updated_dt, description, \
                                image_ipfs_url, image_web_url, ipfs_image_cid, meta_data, meta_data_ipfs_cid, meta_data_ipfs_url, mint_gas_fee, mint_success, name, title, view_public, contract_id, token_id))
            self.commit()
    ...
    def add_token (self, createdby=1, token_id=1, created_dt=datetime.now(), last_modified_by=1, updated_dt=None,  best_ask=0, best_bid=0,featured=True, public_view=True,  \
        ...
        for n in range(start_bulk, n_rows + 1):
            self.cur.execute("INSERT INTO TOKEN (created_by, created_dt, last_modified_by, updated_dt, active, best_ask, best_bid, featured, fotd, public_view, trending, chain_id) " \
                            "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", \
                             (createdby, created_dt, last_modified_by, updated_dt, active, best_ask, best_bid, featured, fotd, public_view, trending, chain_id))
            self.commit()
